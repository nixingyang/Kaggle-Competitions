...
def load_data():
    ...
    # Feature engineering
    combined_file_content.drop(["AnimalID", "OutcomeSubtype"],
                               axis=1,
                               inplace=True)

    combined_file_content["has_no_name"] = pd.isnull(
        combined_file_content["Name"])
    ...
    datetime_list = [
        parse(current_record)
        for current_record in combined_file_content['DateTime']
    ]
    ...
    combined_file_content[
        'age_in_months'] = combined_file_content.AgeuponOutcome.apply(
            get_age_in_months)

    combined_file_content["mixed_breed"] = [
        "Mix" in current_record
        for current_record in combined_file_content['Breed']
    ]
    combined_file_content.drop(["Breed"], axis=1, inplace=True)

    combined_file_content["mixed_color"] = [
        "/" in current_record
        for current_record in combined_file_content['Color']
    ]
