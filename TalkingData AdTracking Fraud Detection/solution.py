...
def release_resources():
    ...
    print(
        "Collected {} unreachable objects ...".format(unreachable_objects_num))
...
def load_data(nrows=SAMPLE_NUM):
    ...
    temp_df = merged_df[["ip", "day", "hour",
                         "channel"]].groupby(by=["ip", "day", "hour"])[[
                             "channel"
    ...
    temp_df = merged_df[["ip", "app", "channel"]].groupby(by=["ip", "app"])[[
        "channel"
    ...
    temp_df = merged_df[["ip", "app", "os",
                         "channel"]].groupby(by=["ip", "app", "os"])[[
                             "channel"
                         ]].count().reset_index().rename(
    ...
    temp_df = merged_df[["ip", "day", "hour",
                         "channel"]].groupby(by=["ip", "day", "channel"])[[
                             "hour"
    ...
    temp_df = merged_df[["ip", "app",
                         "os", "hour"]].groupby(by=["ip", "app", "os"])[[
                             "hour"
                         ]].var().reset_index().rename(
    ...
    temp_df = merged_df[[
        "ip", "app", "channel", "day"
    ]].groupby(by=["ip", "app", "channel"])[["day"]].var().reset_index().rename(
    ...
    temp_df = merged_df[["ip", "app", "channel",
                         "hour"]].groupby(by=["ip", "app", "channel"])[[
                             "hour"
    ...
    train_indexes, valid_indexes = train_test_split(np.arange(train_num),
                                                    test_size=0.1,
                                                    random_state=0)
    train_df, valid_df = merged_df.iloc[train_indexes], merged_df.iloc[
        valid_indexes]
...
def run():
    ...
    train_dataset = lgb.Dataset(train_df.drop(target_name, axis=1),
                                train_df[target_name],
                                categorical_feature=categorical_feature)
    train_df = None
    release_resources()
    valid_dataset = lgb.Dataset(valid_df.drop(target_name, axis=1),
    ...
    prediction_array = model.predict(test_df,
    ...
    print("Saving compressed submission to {} ...".format(
        compressed_submission_file_path))
