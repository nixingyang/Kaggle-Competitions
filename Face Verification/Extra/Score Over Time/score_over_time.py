...
pylab.figure()
for TeamName, DisplayName, color in zip(TeamName_list, DisplayName_list,
                                        color_list):
    # Retrieve records
    selected_indexes = np.array(file_content[" TeamName"] == TeamName)
    selected_records = file_content.as_matrix(
        [" DateSubmittedUtc", " PrivateScore"])[selected_indexes, :]
