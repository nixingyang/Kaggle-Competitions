...
def load_data():
    ...
    for categorical_feature_column in categorical_feature_column_list:
        ...
        combined_file_content[
            categorical_feature_column] = label_encoder.fit_transform(
                combined_file_content[categorical_feature_column])
        onehot_encoding_mask.append(len(label_encoder.classes_) > 2)
    sparse_categorical_feature_array = OneHotEncoder(
        categorical_features=onehot_encoding_mask, dtype=np.bool,
        sparse=True).fit_transform(
            combined_file_content[categorical_feature_column_list])
    combined_file_content.drop(categorical_feature_column_list,
                               axis=1,
                               inplace=True)

    # Process continuous features
    combined_file_content[continuous_feature_column_list] = StandardScaler(
    ).fit_transform(combined_file_content[continuous_feature_column_list])
    continuous_feature_array = combined_file_content[
        continuous_feature_column_list].as_matrix()
    combined_file_content.drop(continuous_feature_column_list,
                               axis=1,
                               inplace=True)

    # Combine categorical and continuous features
    X_array = np.hstack((sparse_categorical_feature_array.toarray(),
                         continuous_feature_array)).astype(np.float32)
...
def run():
    ...
    for cross_validation_index, (train_index, valid_index) in enumerate(
        ...
        if not os.path.isfile(optimal_weights_path):
            ...
            modelcheckpoint_callback = ModelCheckpoint(optimal_weights_path,
                                                       monitor="val_loss",
                                                       save_best_only=True)
            model.fit(
                X_train[train_index],
