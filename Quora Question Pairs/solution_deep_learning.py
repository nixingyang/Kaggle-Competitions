...
EMBEDDING_FILE_PATH = os.path.join(PROJECT_FOLDER_PATH,
                                   "glove.42B.300d_word2vec.txt")
DATASET_FILE_PATH = os.path.join(PROJECT_FOLDER_PATH,
                                 "deep_learning_dataset.npz")
...
def correct_typo(word, word_to_index_dict, known_typo_dict, min_word_length=8):
    ...
    def get_candidate_word_list(word):
        ...
        replaced_word_list = [
            ...
            for left_word, right_word in left_word_with_right_word_list
            if right_word for character in ascii_lowercase
        ]
        inserted_word_list = [
        ...
        return list(
            set(deleted_word_list + transposed_word_list + replaced_word_list +
                inserted_word_list))
    ...
    if len(candidate_word_with_index_array) == 0:
        ...
        selected_candidate_word = candidate_word_with_index_array[np.argmin(
            candidate_word_with_index_array[:, -1].astype(np.int))][0]
...
def clean_sentence(original_sentence,
                   word_to_index_dict,
                   known_typo_dict,
                   result_when_failure="empty"):
    ...
    try:
        ...
        # Replace elements
        cleaned_sentence = re.sub(r"[^A-Za-z0-9^,!.\/'+-=]", " ",
                                  cleaned_sentence)
        ...
        cleaned_sentence = "".join([
            character for character in cleaned_sentence
...
def load_file(original_file_path, word_to_index_dict, known_typo_dict):
    ...
    if os.path.isfile(processed_file_path):
        ...
        interesting_column_name_list = [
            "processed_question1", "processed_question2"
        ]
...
def load_dataset():
    if os.path.isfile(DATASET_FILE_PATH):
        ...
        word2vec = KeyedVectors.load_word2vec_format(EMBEDDING_FILE_PATH,
                                                     binary=False)
        word_to_index_dict = dict([
            (word, index) for index, word in enumerate(word2vec.index2word)
        ])
        print("word2vec contains {} unique words.".format(
        ...
        tokenizer.fit_on_texts(train_text_1_list + train_text_2_list +
                               test_text_1_list + test_text_2_list)
        print("Dataset contains {} unique words.".format(
        ...
        train_data_1_array = pad_sequences(train_sequence_1_list,
                                           maxlen=MAX_SEQUENCE_LENGTH,
                                           padding="post",
                                           truncating="post")
        train_data_2_array = pad_sequences(train_sequence_2_list,
                                           maxlen=MAX_SEQUENCE_LENGTH,
                                           padding="post",
                                           truncating="post")
        test_data_1_array = pad_sequences(test_sequence_1_list,
                                          maxlen=MAX_SEQUENCE_LENGTH,
                                          padding="post",
                                          truncating="post")
        test_data_2_array = pad_sequences(test_sequence_2_list,
                                          maxlen=MAX_SEQUENCE_LENGTH,
                                          padding="post",
                                          truncating="post")
        ...
        embedding_matrix = np.zeros(
            (len(tokenizer.word_index) + 1, word2vec.vector_size),
            dtype=np.float32)
...
def init_model(embedding_matrix, learning_rate=0.002):
    ...
    def get_sentence_feature_extractor(embedding_matrix):
        ...
        output_tensor = Embedding(input_dim=embedding_matrix.shape[0],
                                  output_dim=embedding_matrix.shape[1],
                                  input_length=None,
                                  mask_zero=True,
                                  weights=[embedding_matrix],
                                  trainable=False)(input_tensor)
        output_tensor = LSTM(output_dim=256,
                             dropout_W=0.3,
                             dropout_U=0.3,
                             activation="tanh",
                             return_sequences=False)(output_tensor)
    ...
    sentence_feature_extractor = get_sentence_feature_extractor(
        embedding_matrix)
    ...
    output_tensor = merge([output_1_tensor, output_2_tensor],
                          mode="concat",
                          concat_axis=1)
    output_tensor = Lambda(lambda x: K.mean(x, axis=1, keepdims=True),
                           output_shape=(1,))(output_tensor)
    ...
    plot(sentence_feature_extractor,
         to_file=os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH,
                              "sentence_feature_extractor.png"),
         show_shapes=True,
         show_layer_names=True)
    plot(binary_classifier,
         to_file=os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH,
                              "binary_classifier.png"),
         show_shapes=True,
         show_layer_names=True)
    plot(model,
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        pylab.savefig(
            os.path.join(OUTPUT_FOLDER_PATH,
                         "loss_curve_{}.png".format(self.split_index)))
        pylab.close()
        ...
        pylab.savefig(
            os.path.join(OUTPUT_FOLDER_PATH,
                         "accuracy_curve_{}.png".format(self.split_index)))
        pylab.close()
...
def run():
    ...
    for split_index, (train_index_array, valid_index_array) in enumerate(
        ...
        optimal_weights_file_path = os.path.join(
            OPTIMAL_WEIGHTS_FOLDER_PATH,
            "optimal_weights_{}.h5".format(split_index))
        if os.path.isfile(optimal_weights_file_path):
            ...
            actual_train_data_1_array, actual_train_data_2_array, actual_train_label_array = train_data_1_array[
                train_index_array], train_data_2_array[
                    train_index_array], train_label_array[train_index_array]
            actual_valid_data_1_array, actual_valid_data_2_array, actual_valid_label_array = train_data_1_array[
                valid_index_array], train_data_2_array[
                    valid_index_array], train_label_array[valid_index_array]
            ...
            valid_sample_weights = np.ones(
                len(actual_valid_label_array)) * valid_class_weight[1]
            valid_sample_weights[np.logical_not(
                actual_valid_label_array)] = valid_class_weight[0]
            ...
            model.fit([actual_train_data_1_array, actual_train_data_2_array],
                      actual_train_label_array,
                      batch_size=BATCH_SIZE,
                      validation_data=([
                          actual_valid_data_1_array, actual_valid_data_2_array
                      ], actual_valid_label_array, valid_sample_weights),
                      callbacks=[
                          earlystopping_callback, modelcheckpoint_callback,
        ...
        prediction_array = model.predict([test_data_1_array, test_data_2_array],
                                         batch_size=BATCH_SIZE,
                                         verbose=2)
