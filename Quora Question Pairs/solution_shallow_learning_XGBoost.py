...
def load_extra_features():
    ...
    train_file_path_list = sorted(glob.glob(os.path.join(EXTRA_FEATURES_FOLDER_PATH, "*_train_features.csv")))
    test_file_path_list = sorted(glob.glob(os.path.join(EXTRA_FEATURES_FOLDER_PATH, "*_test_features.csv")))
    for train_file_path, test_file_path in zip(train_file_path_list, test_file_path_list):
...
def run():
    ...
    best_params = {"subsample": 0.9, "colsample_bytree": 0.9, "objective": "binary:logistic", "eval_metric": "logloss"}  # Use empirical parameters
    for split_index, (train_index_array, valid_index_array) in enumerate(cv_object.split(np.zeros((len(train_label_array), 1)), train_label_array), start=1):
        ...
        actual_train_data = xgb.DMatrix(actual_train_feature_array, label=actual_train_label_array, weight=actual_train_weight_list)
        ...
        actual_valid_data = xgb.DMatrix(actual_valid_feature_array, label=actual_valid_label_array, weight=actual_valid_weight_list)

        print("Performing the training procedure ...")
        model = xgb.train(params=best_params, dtrain=actual_train_data, evals=[(actual_valid_data, "valid")], num_boost_round=NUM_BOOST_ROUND, early_stopping_rounds=EARLY_STOPPING_ROUNDS)

        print("Performing the testing procedure ...")
        prediction_array = model.predict(xgb.DMatrix(test_feature_array), ntree_limit=model.best_ntree_limit)
