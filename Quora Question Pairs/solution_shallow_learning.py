...
SHALLOW_FEATURES_FILE_PATH = os.path.join(PROJECT_FOLDER_PATH,
                                          "shallow_features.npz")
...
TFIDF_VECTORIZER.fit(
    pd.Series(
        ...
        TRAIN_FILE_CONTENT["question2"].tolist() +
        TEST_FILE_CONTENT["question2"].tolist()).astype(str))

get_noun_set = lambda question: set([
    word for word, tag in pos_tag(word_tokenize(question.lower()))
    if tag[:1] in ["N"]
])
...
def get_handmade_feature(question1, question2, is_duplicate):
    ...
    entry["question_len_diff"] = abs(entry["question1_len"] -
                                     entry["question2_len"])
    ...
    if higher_question_len != 0:
        ...
        entry["question_len_ratio_log"] = np.log(entry["question_len_ratio"] +
                                                 1)
    ...
    entry["question_char_num_diff"] = abs(entry["question1_char_num"] -
                                          entry["question2_char_num"])
    ...
    entry["question_word_num_diff"] = abs(entry["question1_word_num"] -
                                          entry["question2_word_num"])

    # Calculate average word length
    if entry["question1_word_num"] != 0 and entry["question2_word_num"] != 0:
        entry["question1_average_word_length"] = entry[
            "question1_char_num"] / entry["question1_word_num"]
        entry["question2_average_word_length"] = entry[
            "question2_char_num"] / entry["question2_word_num"]
    ...
    for interrogative in [
        ...
        entry["question1_" +
              interrogative] = int(interrogative in question1.lower())
        entry["question2_" +
              interrogative] = int(interrogative in question2.lower())
        entry["question_" +
              interrogative] = int(entry["question1_" + interrogative] and
                                   entry["question2_" + interrogative])
    ...
    if len(union_word_set) != 0:
        entry["word_jaccard_index"] = len(intersection_word_set) / len(
            union_word_set)
        entry["word_jaccard_index_log"] = np.log(entry["word_jaccard_index"] +
                                                 1)
    ...
    if len(union_noun_set) != 0:
        entry["noun_jaccard_index"] = len(intersection_noun_set) / len(
            union_noun_set)
        entry["noun_jaccard_index_log"] = np.log(entry["noun_jaccard_index"] +
                                                 1)
    ...
    if max(len(question1_word_list), len(question2_word_list)) != 0:
        entry["same_word_ratio"] = sum([
            question1_word == question2_word for question1_word, question2_word
            in zip(question1_word_list, question2_word_list)
    ...
    if "question1_stopword_ratio" in entry and "question2_stopword_ratio" in entry:
        entry["question_stopword_ratio_diff"] = abs(
            entry["question1_stopword_ratio"] -
            entry["question2_stopword_ratio"])

    # Calculate the neighbour word pairs
    question1_neighbour_word_set = set([
        word_tuple
        for word_tuple in zip(question1_word_list, question1_word_list[1:])
    ])
    question2_neighbour_word_set = set([
        word_tuple
        for word_tuple in zip(question2_word_list, question2_word_list[1:])
    ])
    if len(question1_neighbour_word_set) + len(
            question2_neighbour_word_set) != 0:
        ...
        entry["neighbour_word_ratio"] = len(intersection_neighbour_word_set) / (
            len(question1_neighbour_word_set) +
            len(question2_neighbour_word_set))
    ...
    intersection_non_stopword_weight_list = [
        WORD_TO_WEIGHT_DICT.get(word, 0)
        for word in intersection_non_stopword_set
    ]
    ...
    non_stopword_weight_ratio_denominator = np.sum(
        total_non_stopword_weight_list)
    non_stopword_num_ratio_denominator = len(question1_non_stopword_set) + len(
        question2_non_stopword_set) - len(intersection_non_stopword_set)
    cosine_denominator = np.sqrt(
        np.dot(question1_non_stopword_weight_list,
               question1_non_stopword_weight_list)) * np.sqrt(
                   np.dot(question2_non_stopword_weight_list,
                          question2_non_stopword_weight_list))
    entry["intersection_non_stopword_num"] = len(intersection_non_stopword_set)
    if non_stopword_weight_ratio_denominator != 0:
        entry["non_stopword_weight_ratio"] = np.sum(
            intersection_non_stopword_weight_list
    ...
    if non_stopword_num_ratio_denominator != 0:
        entry["non_stopword_num_ratio"] = len(
            intersection_non_stopword_set) / non_stopword_num_ratio_denominator
    if cosine_denominator != 0:
        entry["cosine"] = np.dot(
            intersection_non_stopword_weight_list,
            intersection_non_stopword_weight_list) / cosine_denominator
    ...
    entry["intersection_paired_questions_num"] = len(
        intersection_paired_questions)
    entry["question_paired_questions_num_diff"] = abs(
        entry["question1_paired_questions_num"] -
        entry["question2_paired_questions_num"])
    entry["question1_intersection_paired_questions_ratio"] = entry[
        "intersection_paired_questions_num"] / entry[
            "question1_paired_questions_num"]
    entry["question2_intersection_paired_questions_ratio"] = entry[
        "intersection_paired_questions_num"] / entry[
            "question2_paired_questions_num"]
    ...
    entry["sequence_matcher_ratio"] = get_sequence_matcher_ratio(
        question1, question2)
    ...
    if "question1_TFIDF_sum" in entry and "question2_TFIDF_sum" in entry:
        entry["question_TFIDF_sum_diff"] = abs(entry["question1_TFIDF_sum"] -
                                               entry["question2_TFIDF_sum"])
        entry["question_TFIDF_mean_diff"] = abs(entry["question1_TFIDF_mean"] -
                                                entry["question2_TFIDF_mean"])
        entry["question_TFIDF_std_diff"] = abs(entry["question1_TFIDF_std"] -
                                               entry["question2_TFIDF_std"])
        entry["question_TFIDF_num_diff"] = abs(entry["question1_TFIDF_num"] -
...
def get_magic_feature(file_content):
    ...
    all_question_list = file_content["question1"].tolist(
    ) + file_content["question2"].tolist()
    all_unique_question_list = list(set(all_question_list))
    question_to_id_dict = pd.Series(range(len(all_unique_question_list)),
                                    index=all_unique_question_list).to_dict()
    ...
    file_content["question_frequency_diff"] = abs(
        file_content["question1_frequency"] -
        file_content["question2_frequency"])
...
def load_extra_features():
    ...
    def _load_extra_features(train_file_path, test_file_path):
        ...
        if "mephistopheies" in os.path.basename(train_file_path):
            ...
            question1_feature_column_name_list = sorted([
                ...
                if "q1" in column_name
            ])
            question2_feature_column_name_list = sorted([
                column_name for column_name in column_name_list
                if "q2" in column_name
            ])
        common_feature_column_name_list = sorted(
            set(column_name_list) - set(question1_feature_column_name_list +
                                        question2_feature_column_name_list))
        train_question1_feature_array = train_file_content[
            question1_feature_column_name_list].as_matrix().astype(np.float32)
        train_question2_feature_array = train_file_content[
            question2_feature_column_name_list].as_matrix().astype(np.float32)
        train_common_feature_array = train_file_content[
            common_feature_column_name_list].as_matrix().astype(np.float32)
        test_question1_feature_array = test_file_content[
            question1_feature_column_name_list].as_matrix().astype(np.float32)
        test_question2_feature_array = test_file_content[
            question2_feature_column_name_list].as_matrix().astype(np.float32)
        test_common_feature_array = test_file_content[
            common_feature_column_name_list].as_matrix().astype(np.float32)
    ...
    train_file_path_list = sorted(
        glob.glob(
            os.path.join(EXTRA_FEATURES_FOLDER_PATH, "*_train_features.csv")))
    test_file_path_list = sorted(
        glob.glob(
            os.path.join(EXTRA_FEATURES_FOLDER_PATH, "*_test_features.csv")))
    for train_file_path, test_file_path in zip(train_file_path_list,
                                               test_file_path_list):
...
def load_dataset():
    if os.path.isfile(SHALLOW_FEATURES_FILE_PATH):
        ...
        train_common_feature_array = dataset_file_content[
        ...
        test_common_feature_array = dataset_file_content[
        ...
        common_feature_column_name_list = sorted(
            ...
            set(question1_feature_column_name_list +
                question2_feature_column_name_list + ["is_duplicate"]))
        is_train_mask_array = merged_file_content["is_duplicate"] != -1
        train_question1_feature_array = merged_file_content[
            is_train_mask_array][question1_feature_column_name_list].as_matrix(
            ).astype(np.float32)
        train_question2_feature_array = merged_file_content[
            is_train_mask_array][question2_feature_column_name_list].as_matrix(
            ).astype(np.float32)
        train_common_feature_array = merged_file_content[is_train_mask_array][
            common_feature_column_name_list].as_matrix().astype(np.float32)
        train_label_array = merged_file_content[is_train_mask_array][
            "is_duplicate"].as_matrix().astype(np.bool)
        test_question1_feature_array = merged_file_content[np.logical_not(
            is_train_mask_array)][question1_feature_column_name_list].as_matrix(
            ).astype(np.float32)
        test_question2_feature_array = merged_file_content[np.logical_not(
            is_train_mask_array)][question2_feature_column_name_list].as_matrix(
            ).astype(np.float32)
        test_common_feature_array = merged_file_content[np.logical_not(
            is_train_mask_array)][common_feature_column_name_list].as_matrix(
            ).astype(np.float32)
    ...
    for extra_train_question1_feature_array, extra_train_question2_feature_array, extra_train_common_feature_array, \
        extra_test_question1_feature_array, extra_test_question2_feature_array, extra_test_common_feature_array in load_extra_features():
...
def get_augmented_data(question1_feature_array,
                       question2_feature_array,
                       common_feature_array,
                       label_array=None):
    augmented_feature_array = np.vstack(
        (np.hstack((question1_feature_array, question2_feature_array,
                    common_feature_array)),
         np.hstack((question2_feature_array, question1_feature_array,
                    common_feature_array))))
...
def ensemble_predictions(submission_folder_path, proba_column_name):
    ...
    print("There are {} submissions in total.".format(
        len(submission_file_path_list)))
    ...
    for ensemble_func, ensemble_submission_file_name in zip(
        ...
        ensemble_submission_file_content[proba_column_name] = ensemble_func(
            proba_array, axis=0)
        ensemble_submission_file_content.to_csv(ensemble_submission_file_path,
                                                index=False)
...
def run():
    ...
    test_feature_array = get_augmented_data(test_question1_feature_array,
                                            test_question2_feature_array,
                                            test_common_feature_array)
    ...
    for split_index, (train_index_array, valid_index_array) in enumerate(
            cv_object.split(np.zeros((len(train_label_array), 1)),
                            train_label_array),
            start=1):
        ...
        actual_train_data = lgb.Dataset(actual_train_feature_array,
        ...
        actual_valid_data = lgb.Dataset(actual_valid_feature_array,
        ...
        prediction_2_array = model.predict(
            test_feature_array[len(test_feature_array) // 2:],
            num_iteration=model.best_iteration)
        prediction_array = np.mean(np.vstack(
            (prediction_1_array, prediction_2_array)),
                                   axis=0)
    ...
    ensemble_predictions(submission_folder_path=SUBMISSION_FOLDER_PATH,
                         proba_column_name="is_duplicate")
