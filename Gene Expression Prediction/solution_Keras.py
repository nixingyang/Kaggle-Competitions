...
def load_dataset():
    # Read csv files
    x_train = pd.read_csv(os.path.join(DATASET_FOLDER_PATH,
                                       "train/x_train.csv")).as_matrix()
    x_test = pd.read_csv(os.path.join(DATASET_FOLDER_PATH,
                                      "test/x_test.csv")).as_matrix()
    y_train = pd.read_csv(os.path.join(DATASET_FOLDER_PATH,
                                       "train/y_train.csv")).as_matrix()
    ...
    return x_train.astype(np.float32), y_train.astype(
        np.float32), x_test.astype(np.float32)
...
def init_model(input_feature_dim,
               FC_block_num=3,
               FC_feature_dim=256,
               dropout_ratio=0.5,
    ...
    for _ in range(FC_block_num):
        output_tensor = Dense(FC_feature_dim,
...
def run():
    ...
    cv_object = StratifiedShuffleSplit(n_splits=100,
                                       test_size=0.2,
                                       random_state=0)
    for cv_index, (train_index,
                   valid_index) in enumerate(cv_object.split(x_train, y_train),
                                             start=1):
        submission_file_path = os.path.join(
            SUBMISSION_FOLDER_PATH, "submission_{}.csv".format(cv_index))
        if not os.path.isfile(submission_file_path):
            model_file_path = os.path.join(MODEL_FOLDER_PATH,
                                           "model_{}.h5".format(cv_index))
            ...
            submission_file_content = pd.DataFrame(np.hstack(
                (np.expand_dims(np.arange(len(prediction_array)), axis=-1) + 1,
                 prediction_array)),
                                                   columns=[
                                                       "GeneId", "Prediction"
                                                   ])
            submission_file_content.GeneId = submission_file_content.GeneId.astype(
                int)
    ...
    for ensemble_func, ensemble_func_name in zip(
        ...
        submission_file_path = os.path.join(
            ...
            "submission_{}.csv".format(ensemble_func_name))
        if not os.path.isfile(submission_file_path):
