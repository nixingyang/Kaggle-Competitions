...
def init_model(target_num,
    ...
    for _ in range(additional_block_num):
        output_tensor = Convolution2D(additional_filter_num,
                                      3,
                                      3,
                                      subsample=(1, 1),
                                      activation="relu",
                                      border_mode="same")(output_tensor)
        output_tensor = BatchNormalization(mode=0, axis=1)(output_tensor)
    output_tensor = Convolution2D(target_num,
                                  3,
                                  3,
                                  subsample=(1, 1),
                                  activation="linear",
...
def run():
    ...
    unique_label_list = sorted([
        folder_name for folder_name in os.listdir(CROPPED_TRAIN_FOLDER_PATH)
    ...
    if not os.path.isfile(MODEL_WEIGHTS_FILE_PATH):
        ...
        train_generator = load_dataset(CROPPED_TRAIN_FOLDER_PATH,
                                       classes=unique_label_list,
                                       class_mode="categorical",
                                       shuffle=True)
        customizedstopping_callback = CustomizedStopping()
        modelcheckpoint_callback = ModelCheckpoint(MODEL_WEIGHTS_FILE_PATH,
                                                   monitor="loss",
                                                   save_best_only=True,
    ...
    for trial_index in np.arange(TRIAL_NUM) + 1:
        ...
        submission_file_path = os.path.join(SUBMISSION_FOLDER_PATH,
                                            "trial_{}.csv".format(trial_index))
        if not os.path.isfile(submission_file_path):
            print("Performing the testing procedure ...")
            test_generator = load_dataset(CROPPED_TEST_FOLDER_PATH,
                                          shuffle=False)
            ...
            submission_file_content.to_csv(submission_file_path,
                                           header=["image"] + unique_label_list,
                                           index=False)
