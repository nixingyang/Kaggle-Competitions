...
def run():
    ...
    if os.path.isfile(LOOKUP_FILE_PATH):
        ...
        summary_dict = {}
        for before, after in load_text_file(TRAIN_FILE_PATH,
        ...
        for file_path in ADDITIONAL_FILE_PATH_LIST:
            ...
            for before, after in load_text_file(clean_text_file(file_path),
                                                sep="\t",
                                                header=None,
                                                usecols=[1, 2],
        ...
        lookup_file_content = pd.DataFrame(list(lookup_dict.items()),
                                           columns=["before", "after"])
        lookup_file_content.to_csv(LOOKUP_FILE_PATH,
                                   sep="\t",
                                   index=False,
                                   encoding="utf-8")
    ...
    submission_file_path = os.path.join(
        ...
        "{}.csv.gz".format(time.strftime("%c")).replace(" ",
                                                        "_").replace(":", "_"))
