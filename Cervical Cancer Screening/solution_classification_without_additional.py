...
def reorganize_dataset(original_image_path_list, train_index_array,
                       valid_index_array):
...
def init_model(image_height,
    ...
    def get_feature_extractor(input_shape):
        feature_extractor = init_func(include_top=False,
                                      weights="imagenet",
    ...
    def get_dense_classifier(input_shape, unique_label_num):
        ...
        output_tensor = Dense(unique_label_num,
    ...
    plot(feature_extractor,
         to_file=os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH,
                              "{}_feature_extractor.png".format(MODEL_NAME)),
         show_shapes=True,
         show_layer_names=True)
    plot(dense_classifier,
         to_file=os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH,
                              "{}_dense_classifier.png".format(MODEL_NAME)),
         show_shapes=True,
         show_layer_names=True)
    plot(model,
         to_file=os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH,
                              "{}_model.png".format(MODEL_NAME)),
         show_shapes=True,
         show_layer_names=True)
    ...
    assert os.path.isfile(
        previous_optimal_weights_file_path), "Could not find file {}!".format(
...
def load_dataset(folder_path,
    ...
    data_generator_object = ImageDataGenerator(
        ...
        vertical_flip=True,
        preprocessing_function=lambda sample: preprocess_input(
            np.array([sample]))[0])
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        pylab.savefig(
            os.path.join(OUTPUT_FOLDER_PATH,
                         "{}_loss_curve.png".format(MODEL_NAME)))
        pylab.close()
        ...
        pylab.savefig(
            os.path.join(OUTPUT_FOLDER_PATH,
                         "{}_accuracy_curve.png".format(MODEL_NAME)))
        pylab.close()
...
def ensemble_predictions(submission_folder_path):
    ...
    def _ensemble_predictions(ensemble_func, ensemble_submission_file_name):
        ...
        ensemble_proba = ensemble_proba / np.sum(ensemble_proba,
                                                 axis=1)[:, np.newaxis]
        ensemble_submission_file_content.loc[:, proba_columns] = ensemble_proba
        ensemble_submission_file_content.to_csv(os.path.abspath(
            os.path.join(submission_folder_path, os.pardir,
                         ensemble_submission_file_name)),
                                                index=False)
...
def run():
    ...
    original_image_label_list = [
        os.path.basename(
            os.path.abspath(os.path.join(original_image_path, os.pardir)))
    ...
    model = init_model(image_height=IMAGE_HEIGHT,
    ...
    for split_index, (train_index_array, valid_index_array) in enumerate(
            cv_object.split(np.zeros((len(original_image_label_list), 1)),
                            original_image_label_list),
            start=1):
        ...
        if os.path.isfile(optimal_weights_file_path):
            ...
            train_generator = load_dataset(ACTUAL_TRAIN_FOLDER_PATH,
                                           classes=unique_label_list,
                                           class_mode="categorical",
                                           shuffle=True,
                                           seed=SEED)
            valid_generator = load_dataset(ACTUAL_VALID_FOLDER_PATH,
                                           classes=unique_label_list,
                                           class_mode="categorical",
                                           shuffle=True,
                                           seed=SEED)
        ...
        for trial_index in np.arange(TRIAL_NUM) + 1:
            ...
            if not os.path.isfile(submission_file_path):
                print("Performing the testing procedure ...")
                test_generator = load_dataset(TEST_FOLDER_PATH,
                                              shuffle=False,
                                              seed=trial_index)
                prediction_array = model.predict_generator(
                    generator=test_generator,
                    val_samples=len(test_generator.filenames))
                ...
                submission_file_content = pd.DataFrame(
                    np.hstack((image_name_array,
                               prediction_array))[index_array_for_sorting.flat])
                submission_file_content.to_csv(submission_file_path,
                                               header=["image_name"] +
                                               unique_label_list,
                                               index=False)
