...
def get_submission_from_prediction(prediction_file_path_list,
                                   label_index_to_category_id_dict,
                                   ensemble_func):
...
def run():
    ...
    prediction_file_path = os.path.join(
        SUBMISSION_FOLDER_PATH, "{}_prediction_{}.csv".format(
            MODEL_NAME, time.strftime("%c")).replace(" ",
                                                     "_").replace(":", "_"))
    ...
    for image_file_path in image_file_path_list:
        ...
        # Append the results
        entry = [
            tuple(os.path.basename(image_file_path).split(".")[0].split("_"))
    ...
    label_index_to_category_id_dict = dict(
        pd.read_csv(os.path.join(HENGCHERKENG_FOLDER_PATH,
                                 "label_index_to_category_id.csv"),
                    header=None).itertuples(index=False))
    ...
    for ensemble_name, ensemble_func in zip(
        ...
        submission_file_path = os.path.join(
            SUBMISSION_FOLDER_PATH, "ensembling_{}_{}.csv".format(
                ensemble_name,
                time.strftime("%c")).replace(" ", "_").replace(":", "_"))
        ...
        for entry in get_submission_from_prediction(
            [prediction_file_path], label_index_to_category_id_dict,
                ensemble_func):
            # Append the results
