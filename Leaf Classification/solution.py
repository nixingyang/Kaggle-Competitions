...
def retrieve_LBP_feature_histogram(image_path):
    try:
        ...
        print("Unable to retrieve LBP feature histogram in {}.".format(
...
def add_LBP_feature_histogram(file_content):
    ...
    for id_value in id_array:
        image_path = os.path.join(IMAGE_FOLDER_PATH,
                                  "{}{}".format(id_value, IMAGE_EXTENSION))
...
def run():
    ...
    train_X = train_file_content.drop([ID_COLUMN_NAME, LABEL_COLUMN_NAME],
                                      axis=1).as_matrix()
    ...
    for cross_validation_index, (train_index, valid_index) in enumerate(
        ...
        if not os.path.isfile(optimal_weights_path):
            ...
            model.fit(train_X[train_index],
                      ...
                      validation_data=(train_X[valid_index],
                                       train_Y[valid_index]),
        ...
        submission_file_content = pd.DataFrame(test_probabilities,
