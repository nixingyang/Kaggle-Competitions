        ...
        func = {
            ...
            "translateY":
                lambda img, magnitude: img.transform(
                    ...
                    fillcolor=fillcolor),
            "rotate":
                lambda img, magnitude: rotate_with_fill(img, magnitude),  # pylint: disable=unnecessary-lambda
            "color":
                lambda img, magnitude: ImageEnhance.Color(img).enhance(
                    1 + magnitude * random.choice([-1, 1])),
            "posterize":
                lambda img, magnitude: ImageOps.posterize(img, magnitude),  # pylint: disable=unnecessary-lambda
            "solarize":
                lambda img, magnitude: ImageOps.solarize(img, magnitude),  # pylint: disable=unnecessary-lambda
            "contrast":
...
class AutoAugment(DualTransform):  # pylint: disable=abstract-method
    """ Apply data augmentation policies found on ImageNet. """

    def __init__(self, **kwargs):
        super(AutoAugment, self).__init__(**kwargs)
        self.policy = ImageNetPolicy()

    def apply(self, img, **params):
        img = Image.fromarray(np.uint8(img))  # Convert NumPy to PIL
        img = np.array(self.policy(img),
                       dtype=np.float32)  # Convert PIL to NumPy
        return img
