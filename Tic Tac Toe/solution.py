...
def get_augmented_data(feature_array, label_array):
    ...
    for rotate_time, flip_func in product(np.arange(4),
                                          [None, np.fliplr, np.flipud]):
    ...
    return np.array(feature_array_list).reshape(
        -1, feature_array.shape[-1]), np.array(label_array_list).flatten()
...
def ensemble_predictions(submission_folder_path, proba_column_name):
    ...
    for ensemble_func, ensemble_submission_file_name in zip(
        ...
        ensemble_submission_file_content.ix[
            ensemble_submission_file_content[proba_column_name] > 0.5,
            "Category"] = "positive"
...
def run():
    ...
    for split_index, (train_index_array, valid_index_array) in enumerate(
        ...
        actual_train_feature_array, actual_train_label_array = train_feature_array[
            train_index_array], train_label_array[train_index_array]
        actual_valid_feature_array, actual_valid_label_array = train_feature_array[
        ...
        submission_file_content = pd.DataFrame({
            "ID": np.arange(len(prediction_array)),
