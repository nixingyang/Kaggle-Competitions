...
def generate_prediction(estimator,
    ...
    for cv_index, _ in enumerate(range(cv_num), start=1):
        ...
        for cv_fold_index, (train_indexes,
                            validate_indexes) in enumerate(cv_object, start=1):
            print("Working on fold {:d}/{:d} ...".format(
                cv_fold_index, CV_FOLD_NUM))

            cv_X_train, cv_X_validate = X_train[train_indexes], X_train[
                validate_indexes]
            cv_Y_train, cv_Y_validate = Y_train[train_indexes], Y_train[
                validate_indexes]
            temp_estimator = copy.deepcopy(estimator)
            temp_estimator.fit(cv_X_train,
                               cv_Y_train,
            ...
            submission_file_content.to_csv(os.path.join(submission_folder_path,
                                                        submission_file_name),
                                           index=False)

            best_iteration_list.append(best_iteration)
            best_score_list.append(best_score)
            print("The best score {:.4f} is obtained at iteration {:d}.".format(
                best_score, best_iteration))

    print("The median best_iteration={:d} and best_score={:.4f}.".format(
...
def run():
    print("Resetting the submission folder {:s} ...".format(
    ...
    optimal_max_depth, optimal_min_child_weight, optimal_subsample, optimal_colsample_bytree = perform_tuning(
        X_train, Y_train)
