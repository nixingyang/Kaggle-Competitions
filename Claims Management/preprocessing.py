...
def convert_hexavigesimal_value(original_string):
    ...
    weight_list = [
        26**item for item in range(len(original_string) - 1, 0 - 1, -1)
    ]
...
def load_data():
    ...
    combined_file_content.drop([ID_COLUMN_NAME, LABEL_COLUMN_NAME],
                               axis=1,
                               inplace=True)

    # Feature engineering
    combined_file_content["null_num"] = np.sum(pd.isnull(combined_file_content),
                                               axis=1)
    combined_file_content["v22"] = combined_file_content["v22"].apply(
        convert_hexavigesimal_value)
    combined_file_content.drop([
        "v8", "v25", "v36", "v37", "v46", "v51", "v53", "v54", "v63", "v73",
        "v81", "v82", "v89", "v92", "v95", "v105", "v107", "v108", "v109",
